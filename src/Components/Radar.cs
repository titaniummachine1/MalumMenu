...
public sealed class Radar : MonoBehaviour
{
    ...
    private void UpdateBigMapPlayers()
    {
        ...
        for (var i = 0; i < players.Count; i++)
        {
            ...
            var showHalo = CheatToggles.mapImpsHighlight && isImp && !isDead; // mapImpsHighlight is shared
    ...
    {
        if (_mapOverlayRoot == null) return;
        if (!CheatToggles.mapTrails) { DisableAllBigMapTrails(); return; } // mapTrails is shared
    ...
    private void UpdatePlayerUi(PlayerControl p)
    {
        ...
        {
            var highlight = CheatToggles.mapImpsHighlight && isImp && !isDead; // shared
